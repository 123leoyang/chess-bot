...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        {
            if ( accessGrid[loc.getRow()][loc.getCol()] < max
                && accessGrid[loc.getRow()][loc.getCol()] > 0 )
            {
                max = accessGrid[loc.getRow()][loc.getCol()];
            }
        }
        for ( Location loc : locs )
        {
    ...
    {
        accessGrid[getLocation().getRow()][getLocation().getCol()] = -1;
        ArrayList<Location> locs = this.getPossibleMoves( loc );
        for ( Location l : locs )
        {
            accessGrid[l.getRow()][l.getCol()]--;
        }
    }
